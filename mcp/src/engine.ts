// X Algorithm MCP — Core Scoring Engine
// Ported from: weighted_scorer.rs (X's open-source recommendation algorithm)
...
export function scorePost(text: string, options: { hasMedia?: boolean; hasVideo?: boolean; hoursAfterLastPost?: number } = {}): ScoreResult {
  ...
  // ── Penalties ──
  if (!noLinks) penalties.push({ id: 'LINK_IN_POST', ...PENALTIES.LINK_IN_POST });
  if (hashtagCount >= SPAM_HASHTAG_THRESHOLD) penalties.push({ id: 'TOO_MANY_HASHTAGS', ...PENALTIES.TOO_MANY_HASHTAGS });
  if (!strongHook) penalties.push({ id: 'WEAK_HOOK', ...PENALTIES.WEAK_HOOK });
  if (charCount < 50 && charCount > 0) penalties.push({ id: 'TOO_SHORT', ...PENALTIES.TOO_SHORT });
  if (!noMutedWords) penalties.push({ id: 'MUTED_KEYWORD', ...PENALTIES.MUTED_KEYWORD });

  // ── Suggestions ──
  if (!hasMedia) suggestions.push({ label: PENALTIES.NO_MEDIA.label, impact: '+8 pts', fix: PENALTIES.NO_MEDIA.fix, points: 8 });
  if (!hasCTA) suggestions.push({ label: PENALTIES.NO_CTA.label, impact: '+8 pts', fix: PENALTIES.NO_CTA.fix, points: 8 });
  if (!noLinks) suggestions.push({ label: 'Move link to first reply', impact: '+30 pts', fix: PENALTIES.LINK_IN_POST.fix, points: 30 });
  if (!strongHook) suggestions.push({ label: 'Strengthen your opening line', impact: '+12 pts', fix: PENALTIES.WEAK_HOOK.fix, points: 12 });
...
export function generateOptimalSchedule(postCount: number, firstPostHour: number): { time: string; multiplier: number; color: string }[] {
  const schedule = [];
  const spacing = Math.max(2, Math.floor(14 / postCount));
