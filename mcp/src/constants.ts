...
// Extracted from: home-mixer/scorers/weighted_scorer.rs + ranking_scorer.rs
...
export const ENGAGEMENT_WEIGHTS = {
  ...
  quoted_click:    { weight: 1.0,   label: 'Quoted Click',       icon: '🔗',  type: 'positive' as const, source: 'ranking_scorer.rs',   desc: 'Clicks on your post when quoted by someone else.' },
  ...
  vqv:             { weight: 0.3,   label: 'Video Quality View', icon: '🎥', type: 'positive' as const, source: 'weighted_scorer.rs',  desc: 'Quality video view (50%+). Requires MIN_VIDEO_DURATION_MS.' },
  ...
  dwell_time:      { weight: 2.0,   label: 'Dwell (Binary)',     icon: '⏱️', type: 'positive' as const, source: 'weighted_scorer.rs',  desc: 'Binary dwell signal — did user linger? 2× a like.' },
  cont_dwell_time: { weight: 1.5,   label: 'Continuous Dwell',  icon: '⏳',  type: 'positive' as const, source: 'ranking_scorer.rs',   desc: 'Continuous dwell duration — longer reads score higher.' },
  cont_click_dwell:{ weight: 1.5,   label: 'Click-Dwell Time',  icon: '🔍',  type: 'positive' as const, source: 'ranking_scorer.rs',   desc: 'Time spent after clicking to expand — deep interest signal.' },
  ...
  not_dwelled:     { weight: -11.0, label: 'Scrolled Past',     icon: '👋',  type: 'negative' as const, source: 'ranking_scorer.rs',   desc: 'Scrolled past without stopping — −11×.' },
} as const;
